...
signed main(){
    for (int id = 1; id <= 10; id++){
        init();
		freopen(("./own_competition/C/yang_li/interval" + std::to_string(id) + ".in").c_str(), "r", stdin);
		freopen(("./own_competition/C/yang_li/interval" + std::to_string(id) + ".out").c_str(), "w", stdout);
		start();
    }
}