...
signed main(){
	for (int yanglishu = 1; yanglishu <= 10; yanglishu++){
		init();
		freopen(("./own_competition/D/yang_li/gcd" + std::to_string(yanglishu) + ".in").c_str(), "r", stdin);
		freopen(("./own_competition/D/yang_li/gcd" + std::to_string(yanglishu) + ".out").c_str(), "w", stdout);
		start();
	}
	return 0;
}