...
int main(){
    ...
    */
    // freopen("./data_structure/treap/P3369.in", "r", stdin);
    // freopen("./data_structure/treap/P3369.out", "w", stdout);
    int n;
    cin >> n;
    int opt, x;
    while(n--){
        ...
        if (opt == 4) cout << kth(rt, x) << endl;
        if (opt == 5) cout << kth(rt, rank_(rt, x) - 1) << endl;
        if (opt == 6) cout << kth(rt, rank_(rt, x + 1)) << endl;
    }
	return 0;
}
